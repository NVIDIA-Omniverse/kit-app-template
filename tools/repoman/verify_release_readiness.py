...
def extract_major_version_from_branches(branches: List[str]) -> List[str]:
    """Extract major version numbers from branch names like 'prod-109', 'integ-109'."""
    major_versions = []
    for branch in branches:
        # Extract number from branch name (e.g., "109" from "prod-109" or "integ-109")
        match = re.search(r"-(\d+)$", branch)
        if match:
            major_versions.append(match.group(1))

    return list(set(major_versions))  # Remove duplicates
...
def validate_deploy_exts_branch_compatibility(config: Dict) -> List[str]:
    """
    Validate that kit-kernel major version is compatible with repo_deploy_exts branch configuration.

    This check ensures that the kit-kernel major version (e.g., "109") matches the major version
    numbers in the repo_deploy_exts pipeline branch names (e.g., "prod-109", "integ-109").
    This is critical for ensuring extensions are deployed to the correct registry branches
    that match the kit version being used.
    ...
    Example:
        Kit version "109.0.0" should match branches ["prod-109", "integ-109"]
        Kit version "108.5.2" would fail against branches ["prod-109", "integ-109"]
    """
    validation_errors = []
    ...
    kit_version = get_kit_kernel_version()
    kit_major_version = kit_version.split(".")[0]

    print_log(f"Kit-kernel version: {kit_version}")
    print_log(f"Kit major version: {kit_major_version}")
    ...
    if branches:
        ...
        # Extract major versions from branch names
        branch_major_versions = extract_major_version_from_branches(branches)
        print_log(f"Major versions from deploy_exts branches: {branch_major_versions}")

        # Check if kit major version matches any branch major version
        if kit_major_version not in branch_major_versions:
            error_msg = (
                f"Kit-kernel major version '{kit_major_version}' does not match any deploy_exts branch major versions "
                f"{branch_major_versions}. Please update repo_deploy_exts branch configuration in repo.toml "
                f"to match kit-kernel version {kit_version}"
            )
            validation_errors.append(error_msg)
        else:
            print_log(f"PASS: Kit major version {kit_major_version} matches deploy_exts branch configurations")
    else:
